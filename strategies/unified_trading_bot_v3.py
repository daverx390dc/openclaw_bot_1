...
order_history = {}                 # NEW: Track all order IDs with timestamps {order_id: timestamp}
...
async def init_exchange():
    ...
    exchange.enableDemoTrading(False)  # LIVE MODE - using real keys
...
async def reconcile_state():
    ...
    try:
        ...
        # Check both 'positionAmt' (legacy) and 'contracts' (current Binance API)
        exchange_position = next((p for p in positions if float(p.get('contracts', p.get('positionAmt', 0))) != 0), None)
        ...
        # Log detailed order info for monitoring
        if open_orders:
        ...
        if exchange_position and not current_position:
            ...
            # Check if there's a stop order (STOP or STOP_MARKET)
            stop_orders = [o for o in open_orders if 'STOP' in (o.get('type', '') or o.get('info', {}).get('type', '')).upper()]
            if stop_orders:
                stop_order_id = stop_orders[0]['id']
                current_position['sl_price'] = float(stop_orders[0].get('stopPrice') or stop_orders[0].get('info', {}).get('stopPrice') or 0)
                log_state(f"✅ Found existing SL order: {stop_order_id} @ {current_position['sl_price']}")
                print(f"✅ SL order detected via API: {stop_order_id}")
            else:
                log_state("⚠️ WARNING: Position has no stop-loss! Will place one.")
        ...
        if current_position and exchange_position:
            ...
            if len(stop_orders) == 0:
                ...
                # Set emergency SL: 3% away from entry
                if current_position['side'] == 'long':
                    current_position['sl_price'] = current_position['entry_price'] * 0.97  # 3% below entry
                else:
                    current_position['sl_price'] = current_position['entry_price'] * 1.03  # 3% above entry
                log_state(f"Emergency SL set to {current_position['sl_price']:.2f}")
...
async def cancel_previous_trailing_orders():
    """Cancel all previous TRAILING stop-loss orders, keeping only latest"""
    global stop_order_id
    try:
        orders = await exchange.fetch_open_orders(SYMBOL)
        # Get all STOP/STOP_MARKET orders except the current one
        stop_orders = [o for o in orders if 'STOP' in o.get('type', '').upper()]
        
        for order in stop_orders:
            if order['id'] != stop_order_id:  # Don't cancel the latest one
                try:
                    await exchange.cancel_order(order['id'], SYMBOL)
                    log_state(f"Cancelled old trailing SL: {order['id']}")
                except Exception as e:
                    log_state(f"Failed to cancel old SL {order['id']}: {e}")
        
        return True
    except Exception as e:
        log_state(f"cancel_previous_trailing_orders error: {e}")
        return False
...
async def update_trailing_or_close(price, atr):
    ...
    # Skip management if risk is 0 (recovered position with unknown risk)
    if risk == 0:
        log_state("Position recovery: skipping management (risk=0, waiting for recalc)")
        return
    ...
    if updated:
        # Cancel ONLY previous trailing SL orders, not all orders
        success = await cancel_previous_trailing_orders()
        if not success:
        ...
        for attempt in range(3):
            try:
                ...
                sl_exists = any(o['id'] == stop_order_id for o in orders)
                if sl_exists:
                    log_state(f"✅ Updated SL verified in open orders")
                    sl_updated = True
                    break
                else:
                    log_state(f"⚠️ Updated SL created but not verified (attempt {attempt+1})")
                    if attempt < 2:
                        await asyncio.sleep(1)
        ...
        if not sl_updated:
            ...
            print(f"⚠️ SL update failed after 3 attempts")
...
async def close_position(price=None, reason="Manual/SL/TP"):
    ...
    current_position = None
    stop_order_id = None
    order_history.clear()  # Clear all order history
    ...
    while True:
        try:
            ...
            try:
                ...
                # Fallback to polling if watch_ohlcv hangs
                log_state("watch_ohlcv timeout - falling back to fetch_ohlcv")
            ...
            # Manage existing position (only if indicators ready)
